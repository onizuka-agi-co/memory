...
### 🎯 企画：AGI Knowledge Hub 自動コンテンツ配信システム

**背景:**
- Backlog P1タスク3件（ナレッジグラフ系）がDone済みタスクと重複
- 「蓄積」フェーズから「配信」フェーズへの転換が必要
- 多言語化（en/zh）進行中と連携可能

**企画内容:**
1. **コンテンツキュレーション層** — Knowledge Baseから注目論文を自動選定
2. **フォーマット変換層** — X用短文 / Discord用embed / Newsletter用長文を自動生成
3. **スケジュール配信層** — 最適投稿時刻に自動配信
4. **反応分析層** — エンゲージメント追跡・フィードバック

**技術スタック:** Gemini + X API + Discord Webhook + s6定期実行
